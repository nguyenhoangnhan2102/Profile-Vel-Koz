...
//Lấy id_E để UPDATE
const getUpdateEById = async (id_e) => {
    let [results, fields] = await connection.query(
        "SELECT * FROM E WHERE id_e = ?", [id_e],
    );

    let skill_e = results && results.length > 0 ? results[0] : {};

    return skill_e;
};

//Lấy id_R để UPDATE
const getUpdateRById = async (id_r) => {
    let [results, fields] = await connection.query(
        "SELECT * FROM R WHERE id_r = ?", [id_r],
    );

    let skill_r = results && results.length > 0 ? results[0] : {};
...
module.exports = {
    ...
    getUpdateWById, getUpdateEById, getUpdateRById
};